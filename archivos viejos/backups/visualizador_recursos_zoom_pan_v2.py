...
class EditorMaestroSprites:
    ...
    def _get_canvas_coords(self, mx, my):
        """Convierte coordenadas de pantalla a coordenadas de la imagen (con zoom y pan)"""
        ix = (mx - self.rect_canvas.x - self.pan_x) / self.zoom
        iy = (my - self.rect_canvas.y - self.pan_y) / self.zoom
        return ix, iy

    def _get_screen_coords(self, ix, iy):
        """Convierte coordenadas de imagen a coordenadas de pantalla"""
        sx = ix * self.zoom + self.rect_canvas.x + self.pan_x
        sy = iy * self.zoom + self.rect_canvas.y + self.pan_y
        return sx, sy
    ...
    def dibujar(self):
        ...
        if self.img_sheet:
            ...
            w_scaled = int(self.img_sheet.get_width() * self.zoom)
            h_scaled = int(self.img_sheet.get_height() * self.zoom)
            if w_scaled > 0 and h_scaled > 0:
                img_scaled = pygame.transform.scale(self.img_sheet, (w_scaled, h_scaled))
                canvas_surf.blit(img_scaled, (self.pan_x, self.pan_y))
